...
resource "aws_iam_policy" "cluster_autoscaler_iam_policy" {
  name        = "${var.prefix_tag_name}-AmazonEKSClusterAutoscalerPolicy"
  path        = "/"
...
resource "aws_iam_role" "cluster_autoscaler_iam_role" {
  name = "${var.prefix_tag_name}-cluster-autoscaler"
  ...
  tags = {
    tag-key = "${var.prefix_tag_name}-cluster-autoscaler"
  }
}
